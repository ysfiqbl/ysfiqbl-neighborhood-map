$(document).ready(function() {
	...
	var ViewModel = function() {
		...
		self.locationSearchSuggestions = ko.computed(function() {
			...
			if (query === '' && self.showListView() === true) {
				...
				return ko.utils.arrayFilter(self.allLocations, function(loc) {
					...
					if (self.infowindow !== undefined && !result && self.infowindow.marker == marker) {
		...
		self.loadLocations = function() {
			var localStoreLocations;
			var locationJSON;
			var loc;
			if (localStorage.locations && localStorage.locations !== '[]') {
				localStoreLocations = JSON.parse(localStorage.locations);

			} else {
				localStoreLocations = self.initializeData();
			}
		...
		self.foursquareResponseHandler = {
			success: function(obj) {
				...
				if ($.isEmptyObject(venues)) {
					self.address('n/a');
					self.checkinsCount('n/a');
					self.tipCount('n/a');
					self.usersCount('n/a');
				} else {
					var venue = venues[0];
					var stats = venue.stats;
					self.address(venue.location.formattedAddress.join(","));
					self.checkinsCount(stats.checkinsCount || 0);
					self.tipCount(stats.tipCount);
					self.usersCount(stats.usersCount);
				}
				self.showInfo(true);
			},
			fail: function() {
		...
		self.initializeData = function() {
			var data = [
				{"title":"Cemetry","lat":6.844921938032888,"lng":79.86549854278564},
				{"title":"Odel","lat":6.841513169099581,"lng":79.86700057983398},
				{"title":"Berjaya Hotel","lat":6.838594391363253,"lng":79.86378192901611},
				{"title":"Lavinia Breeze","lat":6.840959241849386,"lng":79.86290216445923},
				{"title":"St. Thomas Collega","lat":6.837380004132014,"lng":79.86509084701538},
				{"title":"Mount Grill","lat":6.841640998373764,"lng":79.86721515655518},
				{"title":"Loon Tao","lat":6.83944659108594,"lng":79.86315965652466},
				{"title":"Eagle Lakeside Auditorium","lat":6.84072488781938,"lng":79.8794674873352},
				{"title":"Shore By O","lat":6.838360036173007,"lng":79.86324548721313},
				{"title":"Mount Breeze","lat":6.840320094223907,"lng":79.86300945281982}
			];
